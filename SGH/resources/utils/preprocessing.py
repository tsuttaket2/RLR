...
class Discretizer:
    def __init__(self, timestep=0.8, store_masks=True, impute_strategy='zero', start_time='zero',
        ...
        with open(config_path) as f:
            ...
            self._variable_impute_strategy = config['Imputation_Strategy']
    ...
    def transform(self, X, header=None, end=None):
        if header is None:
            header = self._header
        #Change Here
        #assert header[0] == "DAYS"
        eps = 1e-6

        N_channels = len(self._id_to_channel)
        #Change Here
        ts = [float(row[-1]) for row in X]
        ...
        for row in X:
            ...
            for j in range(0, len(row)):
                ...
                if channel not in self._id_to_channel:
                    continue
            ...
            for bin_id in range(N_bins):
                for channel in self._id_to_channel:
                    ...
                    if mask[bin_id][channel_id] == 1:
                        ...
                        continue
                    #Change Here
                    if self._variable_impute_strategy[channel] == 'normal_value':
                        imputed_value = self._normal_values[channel]
                    #Change Here
                    if self._variable_impute_strategy[channel] == 'previous':
                    ...
                    #Change Here
                    if self._variable_impute_strategy[channel] == 'zero':
                        continue
