...
class SoPa_MLP_reduce_seqdim(Module):
    def __init__(self,
                 input_dim,
                 input_sopa,
                 pattern_specs,
                 ...
                 gpu=True,
                 dropout=0.4):
        super(SoPa_MLP_reduce_seqdim,self).__init__()

        self.deep_supervision=deep_supervision
        self.total_num_patterns = sum(pattern_specs.values())
        
        self.mlp_input = MLP_reduce_dim_seq(input_dim, input_sopa, dropout, dropout)
        self.sopa = SoftPatternClassifier(input_sopa,pattern_specs,semiring,deep_supervision,gpu,dropout)
        ...
        self.input_sopa = input_sopa
        self.input_dim = input_dim
    def forward(self,input,mask):

        if self.deep_supervision :
            input_sopa=self.mlp_input(input)
            s_t=self.sopa(input_sopa,mask,self.dropout)
            ...
            input_sopa=self.mlp_input(input)
            s_doc = self.sopa(input_sopa,mask,self.dropout)
...
class SoftPatternClassifier(Module):
    ...
    def get_transition_matrices(self, batch,dropout):
        b = batch.size()[0]
        n = batch.size()[2]
        assert self.input_dim==batch.size()[1], f'batch.size()[1] is {batch.size()[1]} while model is {self.input_dim}'
...
class MLP_reduce_dim_seq(Module):
    """
    A multilayer perceptron with one hidden ReLU layer.
    Expects an input tensor of size (batch_size, input_dim) and returns
    a tensor of size (batch_size, output_dim).
    ...
    def __init__(self,
                 ...
                 dropout_input=0,
                 dropout_emb=0):
        super(MLP_reduce_dim_seq, self).__init__()

        self.emb_dim =emb_dim
        # create a list of layers of size num_layers
       
        self.embedding = torch.nn.Sequential(
            torch.nn.Dropout(p=dropout_input),
            torch.nn.Linear(input_dim, emb_dim, bias=False),
            torch.nn.Dropout(p=dropout_emb)
        )
    ...
    def forward(self,input):
        ...
        #Seq_len x Batch_Size x Input_Dim 
        
        o_t=torch.zeros((seq_len,Batch_Size,self.emb_dim)).to(device)
        for t,input_t in enumerate(input):
            o_t[t,...] = self.embedding(input_t)
        o_t=o_t.transpose(0,1)
        o_t=o_t.transpose(2,1)
        #Batch_Size x Input_Dim x Seq_len
        return o_t
...
def extract_learned_structure(model):
    ...
    if new_pattern_specs:
        ...
        new_model = SoPa_MLP_reduce_seqdim(input_dim=model.input_dim,
                            input_sopa=model.input_sopa,
...
def evaluate_decomp_deepsup_AUPRC(model, data_gen):
    with torch.no_grad():
        ...
        data_true = []
        data_pred = []
        #val_batch=random.sample(range(280),math.ceil(val_nbatches*args.batch_size/100))
        for each_batch in range(data_gen.steps):
            ...
            for t, p in zip(batch_y.cpu().numpy().flatten(),
                            masked_output.cpu().detach().numpy().flatten()):
                data_true.append(t)
                data_pred.append(p)
...
def train_reg_str(train_data_gen, dev_data_gen, model, num_epochs, learning_rate, run_scheduler, gpu, clip, patience, reg_strength, logging_path, min_epoch):
    ...
    for each_chunk in range(num_epochs):
        ...
        if each_chunk < min_epoch: continue
        model.eval()
        dev_AUPRC = evaluate_decomp_deepsup_AUPRC(model,dev_data_gen)
        print(f"dev_AUPRC : {dev_AUPRC}\t and best_dev_AUPRC : {best_dev_AUPRC}")
        ...
        print(f'unchanged: {unchanged} and pattern_specs = {new_pattern_specs}')
        if unchanged == 0:
