...
class SoPa_MLP(Module):
    def __init__(self,
        ...
        self.mlp = MLP(self.total_num_patterns, mlp_patterns, num_classes,dropout_mlp)
...
class MLP(Module):
    ...
    def __init__(self,
        ...
        self.mlp_patterns = mlp_patterns
        hidden_layer_dim = [int(i) for i in mlp_patterns.split("_")]
        num_layers = len(hidden_layer_dim)+1
        self.num_layers = num_layers
        # create a list of layers of size num_layers
        layers = []
        for i in range(num_layers):
            d1 = input_dim if i == 0 else hidden_layer_dim[i-1]
            d2 = hidden_layer_dim[i] if i < (num_layers - 1) else num_classes
            layer = Linear(d1, d2)
            layers.append(layer)

        self.layers = ModuleList(layers)
        self.dropout_layer=torch.nn.Dropout(p=dropout_mlp)
    def forward(self, x):
        res = self.layers[0](x)
        for i in range(1, len(self.layers)):
            res = self.layers[i](torch.sigmoid(res))
...
def extract_learned_structure(model):
    ...
    if new_pattern_specs:
        ...
        new_model = SoPa_MLP(input_dim=model.input_dim,
