...
ProbSemiring = \
    Semiring(
        ...
        torch.max,
        torch.mul,
        lambda x: torch.sigmoid(torch.sign(x)*torch.log(torch.abs(x)+1.)),
        identity
    )
ProbSemiring2 = \
    Semiring(
...
LogSpaceProbSemiring = \
    Semiring(
        zeros,
        ones,
        torch.add,
        torch.mul,
        lambda x: torch.log(torch.min(torch.sigmoid(1/SCALE*x)+1e-7,torch.tensor([1.]).to(device))),
        torch.exp
    )
