...
def extract_learned_structure(model):
    ...
    if new_pattern_specs:
        ...
        new_model = SoPa_MLP(input_dim=57221,
...
def load_saved_traindata(path,batch_data_x_filename,batch_data_y_filename,batch_data_masks_filename,batch_data_masks_T_filename):
    batch_data={'data':[None,None],'masks_T':None,'masks':None}
    #X
    batch_data['data'][0]=os.path.join(path,batch_data_x_filename)
    batch_data['data'][0]=np.load(batch_data['data'][0])['arr_0']
    #Y
    batch_data['data'][1]=os.path.join(path,batch_data_y_filename)
    batch_data['data'][1]=np.load(batch_data['data'][1])['arr_0']
    #masks
    batch_data['masks']=os.path.join(path,batch_data_masks_filename)
    batch_data['masks']=np.load(batch_data['masks'])['arr_0']
    #masks_T
    batch_data['masks_T']=os.path.join(path,batch_data_masks_T_filename)
    batch_data['masks_T']=np.load(batch_data['masks_T'])['arr_0']
    return batch_data
def load_saved_valdata(path,name,batch):
    batch_data={'data':[None,None],'masks_T':None,'masks':None}
    #X
    batch_data['data'][0]=os.path.join(path,name+'batch_data_data_x_'+str(batch)+'.npz')
    batch_data['data'][0]=np.load(batch_data['data'][0])['arr_0']
    #Y
    batch_data['data'][1]=os.path.join(path,name+'batch_data_data_y_'+str(batch)+'.npz')
    batch_data['data'][1]=np.load(batch_data['data'][1])['arr_0']
    #masks
    batch_data['masks']=os.path.join(path,name+'batch_data_masks_'+str(batch)+'.npz')
    batch_data['masks']=np.load(batch_data['masks'])['arr_0']
    #masks_T
    batch_data['masks_T']=os.path.join(path,name+'batch_data_masks_T_'+str(batch)+'.npz')
    batch_data['masks_T']=np.load(batch_data['masks_T'])['arr_0']
    return batch_data

def evaluate_decomp_deepsup_AUPRC(model, val_saved_data_path,val_nbatches=20,batch_size=25):
    with torch.no_grad():
        model.eval()
        #data_val_loss = []
        data_true = []
        data_pred = []
        val_batch=random.sample(range(280),math.ceil(val_nbatches*batch_size/100))
        for each_batch in val_batch:
            batch_data=load_saved_valdata(val_saved_data_path,'Val_Data_Mortality_10Timestep_',each_batch)
            ...
            batch_data = torch.tensor(batch_data['data'][0], dtype=torch.float32).to(device)
            
            batch_data=batch_data.transpose(1,2)
            cur_output=model.forward(batch_data,batch_mask)
            masked_output=cur_output[batch_mask_T.nonzero(as_tuple=True)]
            
            for m, t, p in zip(batch_mask_T.cpu().numpy().flatten(),
                               batch_y.cpu().numpy().flatten(),
                               cur_output.cpu().detach().numpy().flatten()):
                if np.equal(m, 1):
                    data_true.append(t)
                    data_pred.append(p)
...
def train_reg_str(model, num_epochs, learning_rate,\
                 run_scheduler, gpu, clip, patience, reg_strength, logging_path,\
                 val_saved_data_path, train_saved_data_path,\
                 batch_data_x_name_prefix, batch_data_y_name_prefix,\
                 batch_data_masks_name_prefix, batch_data_masks_T_name_prefix,\
                 train_nbatches=30,val_nbatches=20,batch_size=25):
    ...
    #List all the files available batch_x
    batch_data_x_file=[f for f in os.listdir(train_saved_data_path) if f.find(batch_data_x_name_prefix)!=-1]
    list_training_file=[() for _ in batch_data_x_file]
    for i,f in enumerate(batch_data_x_file):
        list_=[]
        for s in re.split('chunk|_batch|.npz',f):
            if s.isdigit():
                list_.append(int(s))
        list_training_file[i]=tuple(list_)
    
    for each_chunk in range(num_epochs):
        cur_batch_loss = []
        model.train()
        
        #sample and remove from the list
        train_batch=random.sample(list_training_file,train_nbatches)
        list_training_file=list(set(list_training_file)-set(train_batch))
        for each_batch,(chunk,batch) in enumerate(train_batch):
            optimizer.zero_grad()
            batch_data_x_filename=batch_data_x_name_prefix+"chunk"+str(chunk)+"_batch"+str(batch)+".npz"
            batch_data_y_filename=batch_data_y_name_prefix+"chunk"+str(chunk)+"_batch"+str(batch)+".npz"
            batch_data_masks_filename=batch_data_masks_name_prefix+"chunk"+str(chunk)+"_batch"+str(batch)+".npz"
            batch_data_masks_T_filename=batch_data_masks_T_name_prefix+"chunk"+str(chunk)+"_batch"+str(batch)+".npz"
    
            batch_data=load_saved_traindata(train_saved_data_path,batch_data_x_filename,\
                                           batch_data_y_filename,batch_data_masks_filename,\
                                           batch_data_masks_T_filename)
        ...
        del batch_mask
        del batch_mask_T
        dev_AUPRC = evaluate_decomp_deepsup_AUPRC(model,val_saved_data_path,val_nbatches,batch_size)
        ...
        if new_model is not None:
            if new_pattern_specs != model.sopa.pattern_specs:
                new_model_dev_AUPRC = evaluate_decomp_deepsup_AUPRC(new_model, val_saved_data_path,val_nbatches,batch_size)
            else:
        ...
        if unchanged == 0:
            ...
            common_utils.create_directory(logging_path)
